...
/*我的笔记
    1.增加了一个向左滑动返回的方法（通过屏幕事件监测，用到了mVelocityTracker以及decorView，分别是计算移动速度和获取顶层的View的）
    2.将ActivityManager集成到了BaseActivity里
    3.提供了setToolBar的方法
    4.添加和取消rx的监听
*/
...
public abstract class BaseActivity extends AppCompatActivity implements LifeSubscription {
    ...
    private View decorView;//为最顶层的View元素
    private VelocityTracker mVelocityTracker;//主要跟踪触摸屏时间的速率
    ...
    public void touchFinish() {
        super.finish();
        overridePendingTransition(R.anim.alpha_enter, R.anim.alpha_exit);//一个Activity切换到另外一个Activity的动画
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                startX = (int) ev.getRawX();//getRawX是指获取和屏幕最左侧的距离
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                deltaX = endX - startX;
                deltaY = endY - startY;
                if (deltaX > deltaY && startX < getWindow().getDecorView().getWidth() / 32) {//如果X轴的变动量大于Y轴的变动量，并且初始按下的X比屏幕的1/32药效的话
